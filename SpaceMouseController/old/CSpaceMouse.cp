/*
Die Werte für X,Y,Z und A,B,C liegen zwischen -400 und 400; bei maximaler Empfindlichkeit
...
pascal void CSpaceMouse::ASyncCompletion(
...
pascal void CSpaceMouse::DeferredDecode(
	long /*dtParam*/)
{
	int i;
	...
	if (fBuffer[fBufferCount-1]==0x0D)
	{
		switch (fBuffer[0])
		{
			...
			case 'q' : //Empfindlichkeit
				...
				break;	
			case 'z' : //"Zeroing"
				//Später eine Message, dass kalibriert wurde ??
				//und/oder zurücksetzen der trans/rot
				break;
...
SInt8 CSpaceMouse::CharToNibble(char in)
{
	switch (in)
	{
		case 0x30:
...
SInt16 CSpaceMouse::FourCharsToVal(char a3,char a2,char a1,char a0)
{
...
void	CSpaceMouse::DecodeKeys(char a0,char a1,char a2)
{
...
void CSpaceMouse::DecodeMode(char a0)
{
	UInt8	mode;
...
void	CSpaceMouse::DecodePaket(void)
{
	x=inttotrans*FourCharsToVal(fBuffer[0*4+1],fBuffer[0*4+2],fBuffer[0*4+3],fBuffer[0*4+4]);
	y=inttotrans*FourCharsToVal(fBuffer[1*4+1],fBuffer[1*4+2],fBuffer[1*4+3],fBuffer[1*4+4]);
	z=inttotrans*FourCharsToVal(fBuffer[2*4+1],fBuffer[2*4+2],fBuffer[2*4+3],fBuffer[2*4+4]);
	
	a=inttorot * FourCharsToVal(fBuffer[3*4+1],fBuffer[3*4+2],fBuffer[3*4+3],fBuffer[3*4+4]);
	b=inttorot * FourCharsToVal(fBuffer[4*4+1],fBuffer[4*4+2],fBuffer[4*4+3],fBuffer[4*4+4]);
	c=inttorot * FourCharsToVal(fBuffer[5*4+1],fBuffer[5*4+2],fBuffer[5*4+3],fBuffer[5*4+4]);
	//Kontroller aktualisieren
	UpdateControllerMove();
}

void CSpaceMouse::DecodeZeroRad(char a0)
{
...
void CSpaceMouse::DecodeSens(char a1,char a0)
{
...
void CSpaceMouse::UpdateControllerMove ()
{
...
void CSpaceMouse::UpdateControllerButtons ()
{
	TQ3Boolean track2DCursor;
...
OSErr CSpaceMouse::SerialPortInit (
	short port,
	short *serPortIn,
	short *serPortOut)
{
...
OSErr CSpaceMouse::SerialPortTerm (
	short serPortIn,
	short serPortOut)
{
...
OSErr CSpaceMouse::SerialPortSet(
	short port)
{
	OSErr err = noErr;
...
OSErr CSpaceMouse::SerRecvFlush(
	short serPort)
{
	OSErr		err = noErr;
	long		count;
...
OSErr CSpaceMouse::SPCMInit(SInt32 _Port)
{
	OSErr err;
...
OSErr CSpaceMouse::SPCMStartRunning(
	void)
{
	OSErr err;
	char command[16]; 
	...
	memset(fBuffer,0x00,BUFFER_LENGTH); //den grossen Puffer löschen
	...
	fDefTask.dtAddr		= NewDeferredTaskProc(CSpaceMouse::DeferredDecode);
	fDefTask.dtParam	= 0;
	fDefTask.dtReserved	= 0;
	
	/* Set up the serial parameter block */
	fSerParm.ioCompletion	= NewIOCompletionProc(CSpaceMouse::ASyncCompletion);
	...
	//einen ModeSet senden!	//oh 15102000
	count=3;
	command[0]='m';
	command[1]='3';
	command[2]=0x0D;
	...
	//Versionsstring anfordern!	//oh 15102000
	count=3;
	command[0]='v';
	command[1]='Q';
	command[2]=0x0D;
...
OSErr CSpaceMouse::SPCMStopRunning(
	void)
{
	OSErr err = noErr;
...
OSErr CSpaceMouse::SPCMSetSerialPort(SInt32 _Port)
{
	OSErr err = noErr;
...
Boolean CSpaceMouse::SPCMGetIsRunning(
	void)
{
	return fIsRunning;
}
